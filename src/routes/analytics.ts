...
analyticsRouter.get('/api/analytics/summary', requireAuth, async (req: Request, res: Response) => {
  ...
  const [total, byStatusRaw] = await Promise.all([
    prisma.edit.count({ where: { createdAt: { gte: from, lte: to } } }),
    prisma.edit.groupBy({ by: ['status'], where: { createdAt: { gte: from, lte: to } }, _count: true }),
  ]);
  const byStatus: Record<string, number> = { new: 0, in_progress: 0, fixed: 0, no_effect: 0, archived: 0 };
  ...
  const [thisWeek, lastWeek] = await Promise.all([
    prisma.edit.count({ where: { createdAt: { gte: weekAgo, lte: now } } }),
    prisma.edit.count({ where: { createdAt: { gte: twoWeeksAgo, lt: weekAgo } } }),
  ]);
  ...
  const byCategory = await Promise.all(
    categories.map(async (c) => {
      const catTotal = await prisma.edit.count({ where: { categoryId: c.id, createdAt: { gte: from, lte: to } } });
